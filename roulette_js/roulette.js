(function($) {
	...
	function remoteRoulette(elements, speed, duration, startDelay = 0) {
		// Vérifier si tous les paramètres sont présents
		if (!elements || !speed || !duration) {
			console.error("Tous les paramètres sont obligatoires : elements, speed, duration");
			return;
		}
	
		// Définir les options de la roulette
		const options = {
			speed: speed,
			duration: duration,
			startDelay: startDelay,
			element: document.getElementById("roulette-container"), // Remplacer par l'ID de votre élément
			elements: elements,
		};
	
		// Créer une nouvelle instance de Roulette.js
		const roulette = new Roulette(options);
	
		// Démarrer la roulette automatiquement
		roulette.start();
		}

		const elements = ["Pomme", "Banane", "Orange", "Kiwi"];
            const speed = 500; // Vitesse de rotation (en millisecondes)
            const duration = 3000; // Durée de la rotation (en millisecondes)

            remoteRoulette(elements, speed, duration);
